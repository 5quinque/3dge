...
class VoxelHandler:
    ...
    def add_voxel(self):
        if self.voxel_id:
            ...
            # is the new voxel position empty?
            if not result[0]:
                _, voxel_index, _, chunk = result
                chunk.voxels[voxel_index] = self.new_voxel_id
                chunk.mesh.rebuild()

                # if it's an empty chunk, set the `is_empty` flag to False
                if chunk.is_empty:
    ...
    def switch_mode(self):
        ...
        print("interaction mode:", self.interaction_mode)

    def update(self):
        self.ray_cast()
        # if self.ray_cast():
        #     self.app.player.crosshair_color = glm.vec3(1.0, 0.0, 0.0)
        # else:
        #     self.app.player.crosshair_color = glm.vec3(1.0)
    ...
    def ray_cast(self):
        ...
        self.voxel_id = 0
        self.voxel_normal = glm.ivec3(0.0)
        step_dir = -1
